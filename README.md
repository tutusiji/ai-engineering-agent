...
一个面向全栈 Web 应用的多 Agent 工程平台。支持前端、后端、数据库和部署的一站式代码生成。
...
- 将聊天描述、PRD、设计说明、实现文档转成结构化全栈需求（前端+后端+数据库+部署）
- 先生成受约束的实现方案，而不是直接自由生成代码
- 复用内部项目模式、通用组件和目标端约定
- 支持前后端分离架构，自动生成 API 契约、数据模型和部署配置
...
1. 全栈需求结构化
2. 目标 profile 选择（前端+后端+数据库+部署）与页面规划
3. 数据建模与 API 契约设计
4. 前端页面、后端服务、数据库迁移的一站式代码生成
5. loading、debounce、删除确认、通用页面模式的规则检查
